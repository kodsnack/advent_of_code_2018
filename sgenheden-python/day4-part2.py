...
if __name__ == "__main__":
    ...
    for key, events in guard_events.items():
        ...
        print(f"Guard {key} is asleep {sum_mat.max()} times at minute {sum_mat.argmax()}")
        guard_sum.append((key, sum_mat.max(), sum_mat.argmax()))

    guard_sum.sort(key=lambda x: x[1], reverse=True)
    print("\n\n")
    print(f"Guard {guard_sum[0][0]} is asleep {guard_sum[0][1]} times at minute {guard_sum[0][2]}")
