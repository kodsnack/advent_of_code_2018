...
function app (input) {	
    ...
    while (queue.length) {
        ...
        if (room(x, y) < next.steps) {
            continue                    
        }

        if (next.steps < ((room(x, y) === undefined) ? 2e10 : room(x, y))) {
            room(x, y, next.steps)
        }

        if (next.regexp.length === 0) {
            continue
        }

        if (next.regexp[0] === '(') {
            let pipeIndex = firstPipeOnSameLevel(next.regexp.slice(1))
            let endingParenIndex = parenOnSameLevel(next.regexp.slice(1))

            let first = next.regexp.slice(1, 1 + pipeIndex)
            let second = next.regexp.slice(1 + pipeIndex + 1, 1 + endingParenIndex)
            let rest = next.regexp.slice(2 + endingParenIndex)
            ...
            let regexp = next.regexp.slice(pipeIndex + 2, endingParenIndex + 1)  

            while (true) {
                pipeIndex = firstPipeOnSameLevel(regexp)
                ...
                queue.push({
                    x: next.x,
                    y: next.y,
                    regexp: regexp.slice(0, pipeIndex) + rest,
                    steps: next.steps
                })

                regexp = regexp.slice(pipeIndex + 1)
            }
        ...
        if (dir[next.regexp[0]]) {
            let [dx, dy] = dir[next.regexp[0]]
            queue.push({
                x: next.x + dx,
                y: next.y + dy,
                regexp: next.regexp.slice(1),
                steps: next.steps + 1
            })
    ...
    console.log("a)", Object.entries(rooms).sort((a, b) => b[1] - a[1])[0][1])
    console.log("b)", Object.entries(rooms).filter(room => room[1] >= 1000).length)
}