...
function app (input) {  
    ...
    for (let line of input.lines()) {
        let match = line.match(/position=<\s*(.*),\s*(.*)> velocity=<\s*(.*),\s*(.*)>/)
        let [_, x, y, v_x, v_y] = match.map(Number)

        points.push({ x: x, y: y, v: { x: v_x, y: v_y } })
    }
    ...
    for (let time = 1; true; ++time) {
        ...
        if (limits.max.y - limits.min.y < 11) { // Seems arbitrary?
            print()
    ...
    function findLimits () {
        let limits = {
            max: { x: -2e10, y: -2e10 }, // sic!
            min: { x: 2e10, y: 2e10 }, // sic!
        }

        for (let point of points) {
            limits.max.x = Math.max(point.x, limits.max.x)
            limits.max.y = Math.max(point.y, limits.max.y)
            limits.min.x = Math.min(point.x, limits.min.x)
            limits.min.y = Math.min(point.y, limits.min.y)
        }
    ...
    function print () {
        for (let i = limits.min.y; i <= limits.max.y; ++i) {
            let pointsOnRow = points.filter(point => point.y == i)

            for (let j = limits.min.x; j <= limits.max.x; ++j) {
                process.stdout.write(
                    pointsOnRow.find(point => point.x == j) ? '##' : '..'
                );
            }
...
app(`position=< 31351, -51811> velocity=<-3,  5>
